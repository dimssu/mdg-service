...
| Send failure dumped `ApiError.message` / "Could not send message."              | High → **[applied]** | Changed to "Your message didn't go through. Please check your network and try again."                                                                                                | Raw API errors on a patchy network are the #1 "the app is broken, I'll just call instead" moment — the app is the only support channel, so a stuck message must never read as broken. |
...
> Team note: a pump is an organisation with an owner plus one or more managers
> (`dealer-staff` with `title: "Manager"`), each with their own app login and
> their own private support chat. Inviting a teammate issues a manager login;
> records and reports stay shared across the whole pump.
